export class Popup {
  constructor(container) {
   ...
   /** REVIEW: Можно лучше:
   * 
   * Каждый класс должен выполнять строго одну задачу. 
   * 
   * В данном случае, класс Popup должен, возвращающать одну карточку. 
   * В классе должны быть методы create и remove и методы, отвечающие за добавление и удаление обработчиков,
   * весь остальной функционал должен быть вынесен в отдельные классы. 
   * 
   * Реализация создания попапов открытия большой картинки по клику и попапа для редактирования информации о пользователе
   * должны быть вынесены в отдельные классы.  
   * Например, можно создать класс PopupImage, который будет отвечать за все, что касается попапа с большой картинкой.
   * Также для других попапов. 
   * Можно реализовать наследование данного класса от класса Popup, это будет отличной практикой. Также для других попапов.
   * 
